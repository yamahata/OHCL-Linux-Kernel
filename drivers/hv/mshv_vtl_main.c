...
struct mshv_vtl_per_cpu {
	...
	u64 l1_msr_sfmask;
	u64 l1_msr_tsc_aux;
	u64 tsc_deadline_prev[MSHV_VTL_NUM_L2_VM];
	bool msrs_are_guest;
...
static int mshv_vtl_alloc_context(unsigned int cpu)
{
	...
	if (hv_isolation_type_tdx()) {
		...
		for (vm_idx = 1; vm_idx <= MSHV_VTL_NUM_L2_VM; vm_idx++) {
			...
			per_cpu->tsc_deadline_prev[vm_idx - 1] = deadline;
		}
#endif
...
#define TDVPS_TSC_DEADLINE	88ULL
...
static void mshv_vtl_return_tdx_tsc_deadline(struct mshv_vtl_run *vtl_run)
{
	...
	deadline = vtl_run->tdx_context.tsc_deadline;
	 ...
	 */
	if (deadline == MSHV_VTL_TDX_DEADLINE_DISARMED)
		deadline = TDVPS_TSC_DEADLINE_DISARMED;
	if (deadline == ~0ULL)
		deadline = ~0ULL - 1ULL;

	if (deadline != per_cpu->tsc_deadline_prev[vm_idx - 1]) {
		tdg_vp_wr(TDVPS_TSC_DEADLINE + vm_idx, deadline, ~0ULL);
		per_cpu->tsc_deadline_prev[vm_idx - 1] = deadline;
	}
}
...
static void mshv_tdx_tsc_deadline_expired(struct mshv_vtl_run *run)
{
	...
	context->tsc_deadline = MSHV_VTL_TDX_DEADLINE_DISARMED;
	context->tsc_deadline_state |= MSHV_VTL_TDX_DEADLINE_EXPIRED;
	per_cpu->tsc_deadline_prev[vm_idx - 1] = TDVPS_TSC_DEADLINE_DISARMED;
}

static bool mshv_tdx_is_tsc_deadline_expired(struct mshv_vtl_run *run)
{
	struct tdx_vp_context *context = &run->tdx_context;
	u64 tsc_deadline = context->tsc_deadline;

	return tsc_deadline != MSHV_VTL_TDX_DEADLINE_DISARMED &&
